...
export class OpenaiService {
    ...
    async getChatReply(prompt: string, context: ChatMessage[]): Promise<string> {
        const history = context.map((message): ChatCompletionMessageParam => {
            return {
                role:
                    message.type === ChatMessageType.USER_TEXT_MESSAGE
                        ? 'user'
                        : 'assistant',
        ...
        const assistant = {
            role: 'system',
            content:
                'You are a python coding assistant that will help users code in python. You will be inclided to be a helper for the developer and will give coding solutions as if you are pair coding. Make sure you give helpul suggestions and code snippets and explain the code to the user.',
        ...
        const completion: ChatCompletion =
            await this.client.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [assistant, ...history, promptMessage],
            });
